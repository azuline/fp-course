...
instance Monad k => Monad (StateT s k) where
  ...
  -- f =<< (StateT k) = StateT $ \s -> k s >>= \(x, s') -> runStateT (f x) s'
  f =<< (StateT k) = StateT $ k >=> uncurry (runStateT . f)
...
distinct' xs = eval' (filtering checkNotSeen xs) S.empty
  where
    checkNotSeen :: Ord a => a -> State' (S.Set a) Bool
    checkNotSeen x =
      do seen <- getT
         putT $ x `S.insert` seen
...
distinctF xs = evalT (filtering checkNotSeen xs) S.empty
  where
    checkNotSeen :: (Ord a, Num a) => a -> StateT (S.Set a) Optional Bool
    checkNotSeen x =
      do seen <- getT
         StateT . const $ if x <= 100
                          then Full (x `S.notMember` seen, x `S.insert` seen)
                          else Empty
...
instance Monad k => Applicative (OptionalT k) where
  ...
  -- OH MY GOD THIS IS SO UGLY BUT IT WORKS!!!!!
...
distinctG xs = runOptionalT $ evalT (filtering filterF xs) S.empty
  where
    filterF :: (Integral a, Show a) => a -> StateT (S.Set a) (OptionalT (Logger Chars)) Bool
    filterF x =
      do seen <- getT
         StateT . const $
           if x > 100
           then let message = fromString $ "aborting > 100: " P.++ show x
                 in OptionalT $ log1 message Empty
           else let logger = if even x
                             then log1 . fromString $ "even number: " P.++ show x
                             else pure
                 in OptionalT . logger $ Full (x `S.notMember` seen, x `S.insert` seen)
