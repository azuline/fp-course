...
betweenCharTok cl cr p = charTok cl *> p <* charTok cr
...
hex = thisMany 4 (satisfy isHexDigit) >>=
  \xs -> case readHex xs of
            Full n -> valueParser $ chr n
            Empty  -> P . const $ UnexpectedString xs
...
sepby pv ps = sepby1 pv ps ||| pure Nil
...
betweenSepbyComma cl cr p = is cl *> sepby p commaTok <* is cr
