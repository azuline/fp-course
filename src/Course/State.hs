...
instance Applicative (State s) where
  ...
  (State k) <*> (State j) = State $ \s -> let (f, s')  = k s
                                              (x, s'') = j s'
                                           in (f x, s'')
...
firstRepeat xs = eval (findM checkSeen xs) S.empty
  where
    checkSeen :: Ord a => a -> State (S.Set a) Bool
    checkSeen x =
      do seen <- get
         put  $ x `S.insert` seen
         pure $ x `S.member` seen
...
--
-- In contrast, a sad number (not a happy number) is where the
-- sum of the square of its digits never reaches 1 because it
...
isHappy x = contains 1 . firstRepeat $ produce (sum' . map (P.^ 2) . digits) x
  where
    digits :: Integer -> List Integer
    digits 0 = Nil
    digits y = y `mod` 10 :. digits (y `div` 10)
