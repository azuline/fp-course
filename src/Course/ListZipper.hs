...
moveLeftLoop lz =
  case moveLeft lz of
    MLZ (Full lz') -> lz'
    _              -> case reverse . toList $ lz of
                        (x:.xs) -> ListZipper xs x Nil
                        _       -> error "Impossible."
...
moveLeft (ListZipper (l:.ls) x rs) = MLZ . Full $ ListZipper ls l (x:.rs)
...
moveRight (ListZipper ls x (r:.rs)) = MLZ . Full $ ListZipper (x:.ls) r rs
...
moveLeftN n lz@(ListZipper (l:.ls) x rs)
  | n < 0     = moveRightN (-n) lz
  | otherwise = moveLeftN (n - 1) (ListZipper ls l (x:.rs))
...
moveRightN n lz@(ListZipper ls x (r:.rs))
  | n < 0     = moveLeftN (-n) lz
  | otherwise = moveRightN (n - 1) (ListZipper (x:.ls) r rs)
...
moveLeftN' n = go n
  where
    go :: Int -> ListZipper a -> Either Int (ListZipper a)
    go 0 lz                      = Right lz
    go i    (ListZipper Nil _ _) = Left (n - i)
    go i lz@(ListZipper (l:.ls) x rs)
      | i < 0     = moveRightN' (-i) lz
      | otherwise = go (i - 1) (ListZipper ls l (x:.rs))
...
moveRightN' n = go n
  where
    ...
    go i lz@(ListZipper ls x (r:.rs))
      | i < 0     = moveLeftN' (-i) lz
      | otherwise = go (i - 1) (ListZipper (x:.ls) r rs)
...
nth n lz = moveRightN diff lz
  where
    diff = n - length (lefts lz)
...
end lz = case moveRightN (length (rights lz)) lz of
           MLZ (Full lz') -> lz'
           _              -> error "Impossible!"
...
start ::
  ListZipper a
  -> ListZipper a
start lz = case moveLeftN (length (lefts lz)) lz of
             MLZ (Full lz') -> lz'
             _              -> error "Impossible!"
...
instance Applicative MaybeListZipper where
  pure x = isZ $ pure x
  MLZ (Full llz) <*> MLZ (Full rlz) = MLZ . Full $ llz <*> rlz
  _              <*> _              = MLZ Empty
...
instance Extend ListZipper where
  f <<= lz = ListZipper visL (f lz) visR
    where
      visL = unfoldr (\lz' -> mut f <$> toOptional (moveLeft lz')) lz
      visR = unfoldr (\lz' -> mut f <$> toOptional (moveRight lz')) lz
      mut g x = (g x, x)
